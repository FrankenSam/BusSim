...
# programmer via call to reactevent() from mainloop())
...
ftn <- function(simlist){
	return(simlist$totwait/simlist$totjobs)
}

roc <- function(x){
	now = ftn(x$currjob)
	last = ftn(x$currjob-1)
	return(now-last)
}

#change variable names: totwait, totjobs, currjob, varname

# main loop of the simulation
mainloop <- function(simlist,simtimelim,simplot =NULL) {
inc <- 0
while(simlist$currtime < simtimelim) {
	...
	if(simplot != NULL){
		if(inc %%  delta == 0){
			plot(simlist$varname, ftn(simlist$varname))	
		} 
		if(rexp(roc(simlist$varname)) == 0 ){
			break
		}
	}	
      if (simlist$dbg) {
